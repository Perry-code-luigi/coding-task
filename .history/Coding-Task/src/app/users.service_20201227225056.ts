...
export class UsersService {
 subject:any;
  ...
  isAMatch(user, query) {
    ...
    return nameRegex.test(user.name) || usernameRegex.test(user.username) || emailRegex.test(user.email) || latRegex.test(user.address.geo.lat) || longRegex.test(user.address.geo.lng);
  }
